...
class streamManager:
    ...
    def delClient(self, uid):
        if uid in self.D:
            ...
            self.globalStats['connectTimes'].append(
                time.time()-self.D[uid].stats.onlineSince)
            del self.D[uid]
    ...
    def setCurrentStream(self, uid_person, uid_truck, sid):
        if uid_person in self.D and uid_truck in self.D and sid in self.D[uid_truck].stream:
            self.D[uid_person].stats.currentStream = {
                'uniqueID': uid_truck, 'streamID': sid}
            if sid != self.D[uid_person].stats.characterStreamID or uid_person != uid_truck:
                self.setPosition(
                    uid_person, self.D[uid_person].stats.characterStreamID, vector3())

    def getCurrentStream(self, uid):
        if uid in self.D:
            if (
                self.D[uid].stats.currentStream['uniqueID'] in self.D
                and self.D[uid].stats.currentStream['streamID'] in self.D[self.D[uid].stats.currentStream['uniqueID']].stream
            ):
    ...
    def countClients(self):
        # minus one because, internally, we consider the server to be a user
        return len(self.D)-1
...
class Discord_Layer:
    ...
    def __init__(self, streamManager, main, ID):
        ...
        self.main = main
        self.ID = ID
        self.channelID = self.main.settings.getSetting(
            'RoRclients', self.ID, 'discordchannel')
        self._stripRoRColoursReg = re.compile(
    ...
    def __send(self, msg, prefix):
        ...
        asyncio.run_coroutine_threadsafe(channel.send(
            "[%s] %s" % (prefix, msg)), self.main.loop)

    # [chat] <username>: hi
    def sayChat(self, msg, uid):
        self.__send("%s: %s" % (self.sm.getUsername(uid),
                    self.__stripRoRColours(msg)), "chat")
    ...
    def sayJoin(self, uid):
        self.__send("%s (%s) joined the server, using %s %s." % (self.sm.getUsername(uid),  s(
            self.sm.getLanguage(uid)), s(self.sm.getClientName(uid)), s(self.sm.getClientVersion(uid))), "game")
    ...
    def sayStreamReg(self, uid, stream):
        ...
        if invalid:
            self.sayInfo("User **%s** with uid **%s** has spawned a **%s** which is a banned vehicle." %
                         (self.sm.getUsername(uid), uid, s(truckinfo['file'])))
            self.main.queueKick(self.channelID, int(uid))
        else:
            self.__send("%s is now driving a %s  (**%s**)." % (self.sm.getUsername(uid),
                        s(truckinfo['name']), s(truckinfo['file'])), "game")
    ...
    def globalStats(self):
        ...
        self.__send("Since %s, we've seen %d players with %d unique usernames." % (
            time.ctime(s['connectTime']), s['userCount'], len(s['usernames'])), "info")
        self.__send("A player stays on average %.2f minutes, but %d players (%.2f%%) left in less than one minute." % (
            averageTime, playerPeek, (float(playerPeek)/float(s['userCount']))*100), "info")
        self.__send("In total, our players drove %.2f meters, flown %.2f meters, sailed %.2f meters and walked %.2f meters." % (
            float(s['distanceDriven']), float(s['distanceFlown']), float(s['distanceSailed']), float(s['distanceWalked'])), "info")
        self.__send("On average, per player: %.2f driven, %.2f flown, %.2f sailed and %.2f walked." % (float(s['distanceDriven'])/float(s['userCount']), float(
            s['distanceFlown'])/float(s['userCount']), float(s['distanceSailed'])/float(s['userCount']), float(s['distanceWalked'])/float(s['userCount'])), "info")
...
class RoR_Connection:
    ...
    def knockServer(self, host, port):
        ...
        data = "MasterServer"
        try:
            sock.send(struct.pack('IIII'+str(len(data))+'s',
                      MSG2_HELLO, 5000, 0, len(data), str(data)))
    ...
    def connect(self, user, serverinfo):
        ...
        self.logger.debug("Trying to connect to server %s:%d",
                          self.serverinfo.host, self.serverinfo.port)
        try:
            self.socket.connect(
                (u"%s" % self.serverinfo.host, self.serverinfo.port))
        except socket.error as msg:
            self.socket.close()
            self.socket = None
            self.logger.error("Couldn't connect to server %s:%d",
                              self.serverinfo.host, self.serverinfo.port)
            return False
        ...
        if packet.command != MSG2_HELLO:
            self.logger.error(
                "Received %s while expecting MSG2_HELLO Exiting...", commandName(packet.command))
            self.disconnect()
            return False
        ...
        if packet is None:
            self.logger.critical(
                "Server sent nothing, while it should have sent us a welcome message.")
            return False
        ...
        if packet.command != MSG2_WELCOME:
            if packet.command == MSG2_FULL:
                ...
                self.logger.error(
                    "invalid handshake: MSG2_HELLO (server error?)")
        ...
        self.logger.info("joined as '%s' on slot %d with UID %s and auth %d",
                         user.username, user.slotnum, str(user.uniqueID), user.authstatus)
    ...
    def registerStream(self, s):
        ...
        if s.type == TYPE_TRUCK:
            data = struct.pack('4i128s2i60s60s', s.type, s.status, s.origin_sourceid,
                               s.origin_streamid, s.name, s.bufferSize, s.time, s.skin, s.sectionConfig)
        else:
            data = struct.pack('iiii128s128s', s.type, s.status,
                               s.origin_sourceid, s.origin_streamid, s.name, b(str(s.regdata)))
        self.sendMsg(DataPacket(MSG2_STREAM_REGISTER,
                     s.origin_sourceid, s.origin_streamid, len(data), data))
        self.sm.addStream(s)
        self.streamID += 1
    ...
    def unregisterStream(self, streamID):
        ...
        self.sendMsg(DataPacket(MSG2_STREAM_UNREGISTER,
                     self.uid, streamID, len(data), data))
    ...
    def replyToStreamRegister(self, data, status):
        ...
        data_out = struct.pack('iiii128s128s', data.type, status,
                               data.origin_sourceid, data.origin_streamid, data.name, b(data.regdata))
        self.sendMsg(DataPacket(MSG2_STREAM_REGISTER_RESULT,
                     self.uid, data.origin_streamid, len(data_out), data_out))
    ...
    def streamCharacter(self, pos, rot, animMode, animTime):
        ...
        data = struct.pack('i5f10s', CHARACTER_CMD_POSITION,
                           rot.x, rot.y, rot.z, rot.w, animTime, b(animMode))
        self.sendMsg(DataPacket(MSG2_STREAM_DATA, self.uid,
                     self.sm.getCharSID(self.uid), len(data), data))
    ...
    def streamTruck(self, s, streamID, recalcTime=True):
        if recalcTime:
            ...
            theTime = s.time
        data = struct.pack('=IfffIfffIfff{0}s'.format(len(s.node_data)), theTime, s.engine_speed, s.engine_force, s.engine_clutch,
                           s.engine_gear, s.hydrodirstate, s.brake, s.wheelspeed, s.flagmask, s.refpos.x, s.refpos.y, s.refpos.z, s.node_data)
        self.sendMsg(DataPacket(MSG2_STREAM_DATA,
                     self.uid, streamID, len(data), data))
    ...
    def sendChat(self, msg):
        ...
        self.sendMsg(DataPacket(MSG2_UTF_CHAT, self.uid,
                     self.sm.getChatSID(self.uid), len(newMsg), newMsg))
    ...
    def sendUserChat(self, user, msg):
        ...
        self.sendMsg(DataPacket(
            MSG2_GAME_CMD, self.uid, 7, len(newMsg), newMsg))
        ...
        if len(msg) > maxsize:
            ...
            for i in range(0, int(math.ceil(float(len(msg)) / float(maxsize)))):
                if i == 0:
                    ...
                    self.sendMsg(DataPacket(MSG2_UTF_CHAT, self.uid, self.sm.getChatSID(
                        self.uid), len(msga), unicode(msga, errors='ignore').encode('utf-8')))
                else:
                    msga = "| "+msg[maxsize*i:maxsize*(i+1)]
                    self.logger.debug("sending %s", msga)
                    self.sendMsg(DataPacket(MSG2_UTF_CHAT, self.uid, self.sm.getChatSID(
            ...
            self.sendMsg(DataPacket(MSG2_UTF_CHAT, self.uid, self.sm.getChatSID(
                self.uid), len(msg), unicode(msg, errors='ignore').encode('utf-8')))
    ...
    def privChat(self, uid, msg):
        ...
        data = struct.pack("I8000s", uid, unicode(
            msg, errors='replace').encode('utf-8'))
        self.sendMsg(DataPacket(MSG2_UTF_PRIVCHAT, self.uid,
                     self.sm.getChatSID(self.uid), len(data), data))
    ...
    def __packPacket(self, packet):
        if packet.size == 0:
            # just header
            data = struct.pack('IIII', packet.command,
                               packet.source, packet.streamid, packet.size)
        else:
            data = struct.pack('IIII'+str(packet.size)+'s', packet.command,
                               packet.source, packet.streamid, packet.size, b(packet.data))
        return data
    ...
    def sendMsg(self, packet):
        ...
        if (packet.command != MSG2_STREAM_DATA):
            self.logger.debug("S>| %-18s %03d:%02d (%d)" % (commandName(
                packet.command), packet.source, packet.streamid, packet.size))
    ...
    def __start_receive_thread(self):
        ...
        if self.socket is None:
            self.logger.error(
                "Tried to receive on None socket (#ERROR_CON009)")
            return None
        ...
        while self.runCondition:
            ...
            if not command in [MSG2_STREAM_DATA, MSG2_UTF_CHAT, MSG2_NETQUALITY]:
                self.logger.debug("R<| %-18s %03d:%02d (%d)" %
                                  (commandName(command), source, streamid, size))

            self.receivedMessages.put(DataPacket(
                command, source, streamid, size, content))
...
class Client(threading.Thread):
    ...
    def __init__(self, ID, main):
        ...
        self.eh = eventHandler(
            self.sm, self.logger, self.discord, self.server, self.main.settings, self.ID)
        self.fullShutdown = 0
    ...
    def run(self):
        reconnectionInterval = self.main.settings.getSetting(
            'RoRclients', self.ID, 'reconnection_interval')
        reconnectionTriesLeft = self.main.settings.getSetting(
            'RoRclients', self.ID, 'reconnection_tries')
        timeUntilRetry = 0
        ...
        while not self.fullShutdown:
            ...
            if self.server.isConnected():
                self.logger.debug(
                    "Disconnecting as we're trying to connect while still connected.")
            ...
            if not self.fullShutdown and reconnectionTriesLeft > 0:
                ...
                while time.time() < timeUntilRetry:
                    try:
                        data = self.main.RoRqueue[self.ID].get(
                            True, timeUntilRetry-time.time()+1)
                    except queue.Empty:
                        pass
                    else:
        ...
        if reconnectionTriesLeft == 0:
            self.discord.sayError(
                "Unable to reconnect. Exiting RoRclient %s ..." % self.ID)
        elif self.fullShutdown:
            self.discord.sayInfo("RoRclient %s exiting on demand..." % self.ID)
        else:
            self.discord.sayError(
                "RoRclient %s exiting after an unknown error occurred..." % self.ID)

    def bigLoop(self):
        # some default values
        user = user_info_t()
        user.username = self.main.settings.getSetting(
            'RoRclients', self.ID, 'username')
        user.usertoken = self.main.settings.getSetting(
            'RoRclients', self.ID, 'usertoken')
        user.serverpassword = self.main.settings.getSetting(
            'RoRclients', self.ID, 'password')
        user.language = self.main.settings.getSetting(
            'RoRclients', self.ID, 'userlanguage')
        user.clientname = self.main.settings.getSetting(
            'general', 'clientname')
        user.clientversion = self.main.settings.getSetting(
            'general', 'version_num')

        serverinfo = server_info_t()
        serverinfo.host = self.main.settings.getSetting(
            'RoRclients', self.ID, 'host')
        serverinfo.port = self.main.settings.getSetting(
            'RoRclients', self.ID, 'port')
        serverinfo.password = self.main.settings.getSetting(
            'RoRclients', self.ID, 'password')
        ...
        self.discord.sayInfo("Connected to server %s" %
                             s(serverinfo.servername))
        ...
        while self.server.runCondition:
            if not self.server.isConnected():
                ...
                self.discord.sayError(
                    "Lost connection to server (#ERROR_CON003)")
                break
        ...
        if packet.command == MSG2_STREAM_DATA:
            ...
            if (stream.type == TYPE_CHARACTER):
                ...
                if streamData.command == CHARACTER_CMD_POSITION:
                    self.sm.setPosition(
                        packet.source, packet.streamid, streamData.rot)
                    self.sm.setCurrentStream(
                        packet.source, packet.source, packet.streamid)
                elif streamData.command == CHARACTER_CMD_ATTACH:
                    self.sm.setCurrentStream(
                        packet.source, streamData.source_id, streamData.stream_id)
                ...
                if len(packet.data) >= 48:
                    ...
                    self.sm.setPosition(
                        packet.source, packet.streamid, streamData.refpos)
                    self.eh.on_stream_data(packet.source, stream, streamData)

            elif stream == None:
                self.logger.warning("EEE stream %-4s:%-2s not found!" %
                                    (packet.source, packet.streamid))
            ...
            self.eh.on_stream_register_result(
                packet.source, processRegisterStreamData(packet.data))
            ...
            str_tmp = str(packet.data).strip('\0')
            self.discord.sayError('Unhandled message (type: %d, from: %d): %s' % (
                packet.command, packet.source, str_tmp))
    ...
    def checkQueue(self):
        while not self.main.RoRqueue[self.ID].empty():
            try:
                ...
                if data[0] == "disconnect":
                    self.server.sendChat(
                        "%sServices is shutting down/restarting... Be nice while I'm gone! :)" % (COLOUR_CYAN))
                    time.sleep(0.5)
                    self.discord.sayInfo("Disconnecting on demand.")
                    ...
                    self.server.sendChat("%s[%s%s%s@Discord%s]: %s" % (
                        COLOUR_WHITE, COLOUR_GREEN, data[2], COLOUR_RED, COLOUR_WHITE, data[1]))
                elif data[0] == "privmsg":
                    ...
                    if data[1] == "full":
                        if self.server.serverinfo.passworded:
                            ...
                            str_tmp = "Public"
                        self.discord.sayInfo("%s server '%s':" % (
                            str_tmp, s(self.server.serverinfo.servername)))
                        self.discord.sayInfo("running on %s:%d, using %s" % (
                            self.server.serverinfo.host, self.server.serverinfo.port, self.server.serverinfo.protocolversion))
                        self.discord.sayInfo("terrain: %s     Players: %d" % (
                            s(self.server.serverinfo.terrain), self.sm.countClients()))
                    elif data[1] == "short":
                        self.discord.sayInfo("name: '%s' - terrain: '%s' - players: %d" % (
                            s(self.server.serverinfo.servername), s(self.server.serverinfo.terrain), self.sm.countClients()))
                    elif data[1] == "ip":
                        self.discord.sayInfo("name: '%s' - host: %s:%d" % (
                            s(self.server.serverinfo.servername), self.server.serverinfo.host, self.server.serverinfo.port))
                elif data[0] == "stats":
                    pass
                else:
    ...
    def showPlayerList(self):
        ...
        # print header
        self.discord.sayInfo("%s | %3s | %5s | %s" %
                             ("Username", "UID", "Lang", "Current vehicle"))
        ...
        for uid in keys:
            if (uid > 0):
                ...
                if uid == self.server.uid:
                    ...
                    currentVehicle = "%s (%s)" % (
                        s(info['name']), s(info['type']))

                self.discord.sayInfo("%2d %s %s | %s | %5s | %s" % (
                    slotnum,
                    rawAuthToString(self.sm.getAuth(uid)),
                    self.sm.getUsername(uid),
                    str(uid),
                    s(self.sm.getLanguage(uid).replace(b'_', b' ')),
                    currentVehicle))
                noPlayers = False
...
class eventHandler:
    ...
    def on_chat(self, source, message):
        ...
        if a[0] == "-say":
            ...
            self.__sendChat_delayed(
                "This command has been disabled because it's quite useless. Use the !say command instead.")

        elif a[0] == "-ping":
            self.__sendChat_delayed(
                "pong! :) (this bot doesn't provide latency statistics. Try using !ping instead.)")

        elif a[0] == "-pong":
            self.__sendChat_delayed(
                "... what are you doing? That's my text! You're supposed to say -ping, so I can say pong. Not the other way around!!!")

        elif a[0] == "-countdown":
            # Initialize a countdown
            self.__sendChat_delayed("Countdown started by %s" %
                                    self.sm.getUsernameColoured(source))
            self.countDown = 3
            ...
            self.__sendChat_delayed("%s will be right back!" %
                                    self.sm.getUsernameColoured(source))

        elif a[0] == "-afk":
            self.__sendChat_delayed(
                "%s is now away from keyboard! :(" % self.sm.getUsernameColoured(source))

        elif a[0] == "-back":
            self.__sendChat_delayed("%s is now back! :D" %
                                    self.sm.getUsernameColoured(source))

        elif a[0] == "-gtg":
            self.__sendChat_delayed(
                "%s got to go! :( Say bye!" % self.sm.getUsernameColoured(source))

        elif a[0] == "-version":
            self.__sendChat_delayed(
                "version: %s" % self.settings.getSetting('general', 'version_str'))
        ...
        # Roleplay commands
            ...
            self.__sendChat_delayed(
                "This command has been disabled because it's quite useless.")
            ...
            if len(a) > 1:
                self.__sendChat_delayed("%s is requesting law enforcement at %s" % (
                    self.sm.getUsernameColoured(source), a[1]))
            else:
                self.__sendChat_delayed("Usage: -police location")

        elif a[0] == "-ems":
            if len(a) > 1:
                self.__sendChat_delayed("%s is requesting emergency medical services at %s" % (
                    self.sm.getUsernameColoured(source), a[1]))
            else:
            ...
            if len(a) > 1:
                self.__sendChat_delayed("%s is requesting the fire department at %s" % (
                    self.sm.getUsernameColoured(source), a[1]))
            else:
            ...
            if len(a) > 1:
                self.__sendChat_delayed("RiP %s%s%s you have been our greatest source of inspiration and courage!" % (
                    COLOUR_CYAN, a[1], COLOUR_WHITE))
            else:
        ...
        # End roleplay commands
            ...
            self.__sendChat_delayed(
                "Available commands: -version, -countdown, -countdown2, -brb, -afk, -back, -gtg, -police, -ems, -fire, -rip, -kickme, -getpos, !version, !rules, !motd, !vehiclelimit, !boost, !boost2, !boost3, !boost4")
            ...
            self.__sendChat_delayed(
                "Wrong prefix. Correct form is: !vehiclelimit")
            ...
            if not self.sm.getAuth(source) & (AUTH_ADMIN | AUTH_MOD):
                ...
                self.__sendChat_delayed(
                    "Usage: -record <start|stop|pause|continue>")
                pass
            elif args[0] == "start":
                if len(args) > 1:
                    self.__sendChat_delayed(
                ...
                self.__sendChat_delayed("Filename: %s" %
                                        self.sr.stopRecording(source))
            elif args[0] == "pause":
            ...
            if not self.sm.getAuth(source) & (AUTH_ADMIN | AUTH_MOD):
                ...
                if len(args) < 1:
                    self.__sendChat_delayed(
                        "Usage: -playback <start|stop|pause|continue>")
                    pass
                elif args[0] == "start":
                    if len(args) > 1:
                        self.__sendChat_delayed(
                            "Playing... ID = %d" % self.sr.startPlayback(args[1]))
                    else:
                        self.__sendChat_delayed(
                            "Playing... ID = %d" % self.sr.startPlayback('last'))
                elif args[0] == "stop":
            ...
            if len(a) <= 1:
                ...
                self.__sendChat_delayed("%s your position is: (x: %f, y: %f, z: %f)" % (
                    COLOUR_CYAN, pos.x, pos.y, pos.z))
            else:
                try:
                    a[1] = int(a[1])
                except ValueError:
                    ...
                    self.__sendChat_delayed("%s, position of %d is: (%f, %f, %f)" % (
                        self.sm.getUsernameColoured(source), a[1], pos.x, pos.y, pos.z))
        ...
        #               self.__sendChat_delayed("OK")
            ...
            self.__sendChat_delayed(
                "current FPS of Services: %d" % self.lastFps)
    ...
    def frameStep(self, dt):
        ...
        if self.fps % 3 == 0:
            if self.serverID == "nhelens":
                self.server.streamCharacter(
                    ...
                    # (rotx, roty, rotz, rotw)
                    vector4(715.261, 45.511, 2415.434, 0),
                    # animationMode[255]
                    CHAR_IDLE_SWAY,
                    0.3                                   # animationTime
                )
                ...
                self.server.streamCharacter(
                    vector3(0, 0, 0),      # (posx, posy, posz)
                    # (rotx, roty, rotz, rotw)
                    vector4(1877.213, 114.170, 2180.039, 0),
                    # animationMode[255]
                    CHAR_IDLE_SWAY,
                    0.3                                   # animationTime
                )
                ...
                self.server.streamCharacter(
                    vector3(0, 0, 0),      # (posx, posy, posz)
                    # (rotx, roty, rotz, rotw)
                    vector4(2432.702, 507.300, 1713.555, 0),
                    # animationMode[255]
                    CHAR_IDLE_SWAY,
                    0.3                                   # animationTime
                )
        ...
        if self.time_ms > 1.0:
            ...
            # countdown system
            if self.countDown > 0:
                self.server.sendChat("%s          %d" %
                                     (COLOUR_CYAN, self.countDown))
                self.countDown -= 1
            elif self.countDown == 0:
                self.server.sendChat(
                    "%s          0    !!! GO !!! GO !!! GO !!! GO !!!" % COLOUR_CYAN)
                self.countDown -= 1
            ...
            if self.settings.getSetting('RoRclients', self.serverID, 'announcementsEnabled') and self.time_sec % self.settings.getSetting('RoRclients', self.serverID, 'announcementsDelay') == 0:
                self.server.sendChat(
                    "#FFFF00ANNOUNCEMENT: %s"
                    %
                    self.settings.getSetting(
                        'RoRclients',
                        self.serverID,
                        'announcementList',
                        (self.time_sec/self.settings.getSetting('RoRclients',
                         self.serverID, 'announcementsDelay'))
                        % len(self.settings.getSetting('RoRclients', self.serverID, 'announcementList'))))
            ...
            if False:
                ...
                if self.time_sec % 3 == 0:
                    self.server.streamCharacter(
                        # (posx, posy, posz)
                        vector3(2540.9, 100.958, 2140.68),
                        # (rotx, roty, rotz, rotw)
                        vector4(0.000000, 0, 0.000000, 0),
                        # animationMode[255]
                        CHAR_SPOT_SWIM,
                        0.0                                   # animationTime
                    )
                    ...
                    self.server.streamCharacter(
                        ...
                        vector3(2540.9, 100.958, 2140.68),
                        # (rotx, roty, rotz, rotw)
                        vector4(0.000000, 0, 0.000000, 0.0),
                        # animationMode[255]
                        CHAR_SWIM_LOOP,
                        0.0                                   # animationTime
                    )
                    ...
                    self.server.streamCharacter(
                        ...
                        vector3(2540.9, 100.958, 2140.68),
                        # (rotx, roty, rotz, rotw)
                        vector4(0.000000, 358.763, 0.000000, 0),
                        # animationMode[255]
                        CHAR_TURN,
                        0.0                                   # animationTime
                    )
...
class streamRecorder:
    ...
    def saveRecording(self, recording):
        ...
        if recording['filename'] == '[default]':
            recording['filename'] = "%d-%04d-%02d" % (
                time.time(), recording['user'].uniqueID, recording['stream'].origin_streamid)
    ...
    def addToRecording(self, stream, streamData):
        if self.isInRecording(stream.origin_sourceid, stream.origin_streamid):
            self.recordings[stream.origin_sourceid][stream.origin_streamid]['data'].append(
                streamData)
        ...
        for rec in self.playList:
            if rec['playInfo']['playing']:
                ...
                if (time.time()-rec['playInfo']['lastFrame'])*1000 > rec['data'][rec['playInfo']['position']].time-rec['data'][rec['playInfo']['lastPosition']].time-10:
                    # stream the frame
                    self.server.streamTruck(
                        rec['data'][rec['playInfo']['position']], rec['playInfo']['streamID'])

                    # advance 1 frame in time
                    rec['playInfo']['lastFrame'] += float(
                        rec['data'][rec['playInfo']['position']].time - rec['data'][rec['playInfo']['lastPosition']].time)/1000
